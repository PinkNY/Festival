...
const KakaoMap = () => {
  let map; // 지도 객체를 전역에서 접근할 수 있도록 선언
  ...
  const displayMarker = (locPosition, message) => {
    if (!map) return; // 지도가 생성되지 않은 경우 return
      ...
      <div id="map" style={{ width: '100%', height: '500px', position: 'relative' }}>
        ...
        <FaLocationCrosshairs 
          ...
          style={{
            ...
            top: '10px',
            right: '10px',
            zIndex: 10, // 지도 위에 아이콘이 오도록 설정
            cursor: 'pointer',
            fontSize: '24px',
            padding: '5px',
            backgroundColor: 'white', // 배경색을 추가하여 아이콘이 더 잘 보이게 설정
            borderRadius: '50%', // 둥근 모서리 설정
            boxShadow: '0px 0px 5px rgba(0, 0, 0, 0.3)' // 그림자 효과 추가
          }} 
          title="현재 위치로 이동" 
        />
      </div>
